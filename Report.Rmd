...
num_predictors <- names(train2018)[sapply(train2018, is.numeric)]

X <- as.matrix(train2018[, num_predictors])
...
x_seq <- seq(min(pca_df$PC1), max(pca_df$PC1), length.out = 200)
y_seq <- seq(min(pca_df$PC2), max(pca_df$PC2), length.out = 200)

grid <- expand.grid(PC1 = x_seq, PC2 = y_seq)
grid$pred <- predict(svm_pca, newdata = grid)

ggplot() +
  geom_tile(data = grid,
            aes(x = PC1, y = PC2, fill = pred),
            alpha = 0.3) +
  geom_point(data = pca_df,
             aes(x = PC1, y = PC2, color = Wildfire),
             size = 1.2) +
...
```{r}
ggplot(pca_df, aes(x = PC1, y = PC2, color = Wildfire)) +
  geom_point(alpha = 0.5, size = 1.2) +
  scale_color_manual(values = c("No" = "blue", "Yes" = "red")) +
  labs(
    title = "Wildfire Classes in PCA Space",
    x = "PC1",
    y = "PC2"
  ) +
  theme_minimal(base_size = 14)
