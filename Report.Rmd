...
With wildfire (Yes) treated as the positive class, the KNN model achieves precision = 0.684 and recall = 0.383, resulting in an F1-score = 0.491. This indicates the model is relatively conservative in its wildfire predictions, with moderate sensitivity to wildfire events and a noticeable number of missed wildfire cases. Overall accuracy (0.605) is not the primary indicator of success here because the goal is to correctly detect wildfire occurrences under balanced evaluation conditions.
...
The ROC curve for the KNN model illustrates its ability to distinguish between wildfire and non-wildfire cases across different classification thresholds. While the curve remains above the diagonal reference line, indicating performance better than random guessing, it does not show strong separation between the two classes. This suggests that although KNN captures some signal in the data, its overall ranking ability is limited compared to stronger models.
...
```
The SVM achieves precision = 0.669 and recall = 0.507, resulting in an F1-score = 0.577.

This indicates that, under balanced evaluation, the model identifies about half of wildfire events while maintaining moderate precision, but missed wildfire detections remain a key limitation. Accuracy (0.630) is reported for completeness, but recall and F1-score are more informative for assessing wildfire detection performance in this setting
...
svm_auc
```
The SVM ROC curve summarizes how well the model separates wildfire from non-wildfire cases across all possible thresholds using predicted probabilities.

Because ROC performance is threshold-independent, it complements the confusion matrix and recall-based evaluation by showing overall ranking ability rather than performance at a single cutoff. In this report, the ROC curve is used to support model comparison, while final judgment still emphasizes recall and F1-score due to the cost of missed wildfire events.
