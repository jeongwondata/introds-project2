...
Despite its contributions, this study has several limitations. First, the dataset primarily includes climate variables and lacks important contextual information such as vegetation dryness, land cover characteristics, and indicators of human activity. The absence of these factors may limit the models’ ability to fully capture the complex causes of wildfire events.
...
fire_prob <- predict(knn_fit, test_smote, type = "prob") %>%
  bind_cols(test_smote)
...
# ROC curve
rf_prob <- predict((rf_model, test_smote) type = "prob")[,"Yes"]
...
The pruned decision tree produced the same evaluation metrics as the original SMOTE-trained tree. After pruning, the model has an accuracy of about 77 percent, a precision of around 5 percent, and a recall of around 29.5 percent for the “Yes” class. This means pruning simplified the tree structure but did not meaningfully change how the model classifies wildfire events.
